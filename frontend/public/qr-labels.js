...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        console.error("Authentication failed on QR labels page:", error);
    }
});
...
function initializeQrLabelsPage(currentUser) {
    ...
    function renderAssetSelectionTable() {
        ...
        if (allAssets.length === 0) {
            assetTableBody.innerHTML = `<tr><td colspan="4" class="p-4 text-center text-gray-500">No assets found.</td></tr>`;
            return;
        }
...
`;
    ...
    // --- EVENT LISTENERS ---
    document.getElementById('print-labels-btn').addEventListener('click', handlePrint);
