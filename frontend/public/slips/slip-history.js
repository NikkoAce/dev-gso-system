...
function initializeSlipHistoryPage(user) {
    ...
    async function showSlipDetails(slipId) {
        ...
        if (slip.slipType === 'PTR') {
            ...
            // Handle different slip types that fall into this 'else' block
            if (slip.custodian) { // For PAR, ICS, RIS
            ...
            } else if (slip.user) { // For IIRUP, A68
                document.getElementById('modal-slip-custodian').textContent = `Generated by: ${slip.user.name}`;
                document.querySelector('#modal-slip-received').parentElement.classList.add('hidden');
            } else { // Fallback
                document.getElementById('modal-slip-custodian').textContent = 'N/A';
                document.querySelector('#modal-slip-received').parentElement.classList.add('hidden');
            }
